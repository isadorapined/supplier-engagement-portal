...
**Supabase project status:** New — Claude Code will create it via MCP at the start of the build session, after confirming the project name below with the builder.
...
| Detail | Answer |
|--------|--------|
| Proposed project name | `the-corporate-supplier-portal` |
| Confirmed project name | `the-corporate-supplier-portal` — confirmed by the builder |
...
N/A — Access Model is A1. No authentication, no roles, no RLS policies in this version. All read/write access to `companies` and `submissions` goes through the anon key with permissive policies scoped to insert/update-on-match only (see Section 11) — there is no per-supplier data isolation to enforce, since suppliers do not read data back through the portal at all in this version.
...
- [ ] Claude Code builds the `companies` and `submissions` tables per Section 5 — no RLS policies needed beyond the standard anon-key insert/update-on-match access described in Section 6, since there is no login yet
